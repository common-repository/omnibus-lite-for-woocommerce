...
namespace OmnibusVendor\WPDesk\Notice;

use OmnibusVendor\WPDesk\PluginBuilder\Plugin\HookablePluginDependant;
use OmnibusVendor\WPDesk\PluginBuilder\Plugin\PluginAccess;
/**
 * Class AjaxHandler
 *
...
class AjaxHandler implements HookablePluginDependant
{
    ...
    {
        $suffix = \defined('OmnibusVendor\\SCRIPT_DEBUG') && SCRIPT_DEBUG ? '' : '.min';
        wp_register_script(self::SCRIPT_HANDLE, trailingslashit($this->assetsURL) . 'js/notice' . $suffix . '.js', array('jquery'), self::SCRIPTS_VERSION);
    ...
    public function processAjaxNoticeDismiss()
    {
        if (isset($_POST[self::POST_FIELD_NOTICE_NAME])) {
            ...
            update_option(PermanentDismissibleNotice::OPTION_NAME_PREFIX . $noticeName, PermanentDismissibleNotice::OPTION_VALUE_DISMISSED);
            do_action('wpdesk_notice_dismissed_notice', $noticeName, $source);
        }
        if (\defined('OmnibusVendor\\DOING_AJAX') && DOING_AJAX) {
            die;
        }
    }
}
