...
namespace OmnibusVendor\WPDesk\Logger\WC;

use OmnibusVendor\Monolog\Logger;
use OmnibusVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured;
/**
...
class WooCommerceCapture
{
    ...
    {
        return \class_exists(\OmnibusVendor\WooCommerce::class) && \version_compare(\OmnibusVendor\WooCommerce::instance()->version, self::SUPPORTED_WC_VERSION, '>=');
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            ...
            if ($this->isWooCommerceLoggerAvailable()) {
                ...
                $this->isCaptured = \true;
            } elseif (\function_exists('OmnibusVendor\\add_action')) {
