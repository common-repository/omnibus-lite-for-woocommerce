...
use OmnibusVendor\WPDesk\Notice\AjaxHandler;
use OmnibusVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('OmnibusVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
        ...
        public function __construct($upgrade_url, $version, $name, $product_id, $plugin_file, $plugin_dir, $hook_to_updates = \true, $plugin_title = null)
        {
            ...
            $wpdesk_installed_plugins[trailingslashit($plugin_dir) . $plugin_file] = $this;
            //
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('OmnibusVendor\\WP_HTTP_BLOCK_EXTERNAL') && \OmnibusVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('OmnibusVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\OmnibusVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
