...
use OmnibusVendor\WPDesk\License\ServerAddressRepository;
...
class LicenseActivation extends AbstractAction implements Action
{
    ...
    {
        $this->add_error('api_key_check_text', 'api_key_check_error', __('Connection failed to the Subscription Key API server. Try again later.', 'wp-wpdesk-license'), 'error');
    }
    /**
    ...
    {
        $this->add_error('activate_text', 'activate_msg', __('Plugin license activated. ', 'wp-wpdesk-license') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
