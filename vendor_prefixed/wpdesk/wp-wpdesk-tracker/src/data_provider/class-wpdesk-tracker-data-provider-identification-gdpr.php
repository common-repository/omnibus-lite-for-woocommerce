...
if (!\class_exists('OmnibusVendor\\WPDesk_Tracker_Data_Provider_Identification_Gdpr')) {
    /**
    ...
    class WPDesk_Tracker_Data_Provider_Identification_Gdpr implements WPDesk_Tracker_Data_Provider
    {
        ...
        private function get_unique_hash()
        {
            ...
            if (!$hash) {
                $hash = \md5(\uniqid() . \OmnibusVendor\NONCE_SALT);
        ...
        {
            $data = [];
            $data[self::DATA_KEY_DOMAIN_HASH] = \md5(home_url() . \OmnibusVendor\NONCE_SALT);
            $data[self::DATA_KEY_UNIQUE_HASH] = $this->get_unique_hash();
            $data[self::DATA_KEY_EMAIL_HASH] = \md5(apply_filters('wpdesk_tracker_admin_email', get_option('admin_email')) . \OmnibusVendor\NONCE_SALT);
            return $data;
        }
    }
}
